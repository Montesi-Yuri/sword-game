...
FMCPToolResult FMCPTool_Material::ExecuteSetMaterialParameters(const TSharedRef<FJsonObject>& Params)
{
	...
	FString LegacyName;
	const bool bAliasPresent = MaterialInstancePath.IsEmpty() && Params->TryGetStringField(TEXT("material_instance_path"), LegacyName) && !LegacyName.IsEmpty();
	if (bAliasPresent)
	{
...
FMCPToolResult FMCPTool_Material::ExecuteGetMaterialInfo(const TSharedRef<FJsonObject>& Params)
{
	...
	FString LegacyName;
	const bool bAliasPresent = AssetPath.IsEmpty() && Params->TryGetStringField(TEXT("asset_path"), LegacyName) && !LegacyName.IsEmpty();
	if (bAliasPresent)
	{
