...
Two source plugins live under `Plugins/`, both cloned from GitHub (not vendored as git submodules —
see [Source control](../../CLAUDE.md#source-control)):

- **VibeUE** (`kevinpbuckley/VibeUE`) — an **expansion** of Unreal 5.8's own built-in MCP server and
  `ToolsetRegistry`/`AgentSkill` systems (it does not run its own server). Enable **Edit → Plugins →
  Unreal MCP** (auto-enables **Toolset Registry**) and **Editor Tools**, then **Editor Preferences →
  General → Model Context Protocol → Auto Start Server** (or console `ModelContextProtocol.StartServer`).
  Default endpoint **`http://127.0.0.1:8000/mcp`** (configurable). Generate the project's `.mcp.json`
  from the in-editor console with `ModelContextProtocol.GenerateClientConfig ClaudeCode` — it doesn't
  exist yet in this repo, so run that once before expecting Claude Code to see these tools.
- **UnrealClaude** (`Natfii/UnrealClaude`) — an in-editor Claude Code chat panel plus its own MCP
  bridge (`Plugins/UnrealClaude/Resources/mcp-bridge`, Node.js, `npm install` already run) on
  **`http://localhost:3000`** by default (`.env` / `.env.example` in that folder). Requires the Claude
  Code CLI installed and authenticated on the machine running the editor.
- Both are **source plugins built from source** — no prebuilt binaries. `UnrealClaude.uplugin` still
  declares `EngineVersion: 5.7.0` (upstream hasn't published a 5.8 release); it built clean on 5.8 after
  a small patch — see below. Re-check this note if you ever `git pull`/re-clone the plugin.

## ⚠️ UnrealClaude UE 5.8 patch (already applied, will be lost on re-clone)

UE 5.8 changed `FJsonObject::Values` from `TMap<FString, TSharedPtr<FJsonValue>>` to a map keyed by
`UE::FSharedString`, which no longer implicitly converts to `FString`. UnrealClaude 1.5.1 targets 5.7
and iterates that map directly (`for (const auto& Pair : Json->Values) { ...Pair.Key... }`) in 8 call
sites across `MCPToolBase.h`, `AnimationBlueprintUtils.cpp`, `MCPTool_AnimBlueprintModify.cpp`,
`MCPTool_BlueprintModify.cpp`, `MCPTool_Material.cpp`, and `MCPTool_SetProperty.cpp` — this fails to
compile on 5.8 with `C2664`/`C2665`/`C2039` errors. Fixed by wrapping the key:
`FString(Pair.Key.ToView())` (or `.ToUpper()` on that constructed `FString`, not on `Pair.Key` itself,
which has no `ToUpper()`). If a future upstream pull reintroduces this pattern, the same fix applies —
grep the plugin's `Source/` for `->Values)` / `.Values)` iteration and check every `Pair.Key` /
`Field.Key` use.

## Sources of tooling knowledge below

The gotchas in this file were carried over from a prior **UE 5.7 Blueprint-only** project using the
same two plugins — this project is **UE 5.8 and C++-primary** (gameplay logic in
`Source/sword_ue5_8`, with Blueprint content for GameMode/widgets/input assets layered on top, per
[CLAUDE.md](../../CLAUDE.md)). Most of this is VibeUE/UnrealClaude *plugin* behavior, not engine-version
behavior, so it should mostly transfer — but treat anything below as **unverified in this project**
until you actually hit it, especially anything touching JSON/string internals given the 5.8 API change
documented above.
...
VibeUE ships a **lazy-loading skill library** at `Plugins/VibeUE/Content/Skills/` — this install has
`vibeue`, `landscape`, `landscape-auto-material`, and `foliage` skill folders at minimum; **run
`manage_skills(action="list")` once per session** to get the authoritative current list rather than
trusting any list written down here. These are battle-tested and cover most UE-tooling gotchas.
**They are reachable the whole time the VibeUE MCP is connected — no manual activation is needed; you
just have to load them.** Nobody injects them for you, so loading is YOUR responsibility.
...
- Blueprint math on doubles uses `Add_DoubleDouble`, `Multiply_DoubleDouble` — NOT `_FloatFloat`.
...
- `EditorLevelLibrary` / `spawn_actor_from_class` are **deprecated** → `EditorActorSubsystem`.
- Verification gate: after any graph edit, re-read nodes + connections + compile before claiming done.

---

## 1. Gotchas NOT covered by VibeUE skills (carried over, re-verify as hit)
...
On a crash relaunch with a safe-DDC flag/launcher if the project has one (check for a `.bat`). After a
...
- **CDO writes work via `get_default_object` (root AND subobjects).** `get_default_object(gc).set_editor_property(
  ...
  ⚠️ After a CDO **class-default** change you MUST `compile_blueprint(<bp>)` + `save_asset`, or PIE keeps the
  old value — this is exactly what makes a GameMode keep spawning the engine `PlayerController` instead of
  your custom one. Default **subobjects** are writable too — the no-C++ way to raise an inherited native
  component's default: `cdo.character_movement.set_editor_property("max_walk_speed", 2200.0)`. Note
  `set_variable_default_value` returns **False** for class-typed inherited props (use the CDO write instead);
  plain `set_component_property(bp, "CharMoveComp", "MaxWalkSpeed", …)` also returns **False** silently.
- **A malformed `WidgetService` / `execute_python_code` call can HARD-CRASH the editor** — e.g. a
  ...
  `0xC0000005`, NOT a returned error. Recover per §1.7 (relaunch, wait, reconnect MCP).
...
unwanted movement binding, **delete its IA-driven `AddMovementInput` nodes**. This project already has
`IA_Attack`/`IA_Block`-style input assets (see recent commit history) — check existing mappings before
adding new ones.
...
A UMG Button's multicast `OnClicked` needs a real bound-event node; the legacy function-binding route
`bind_event` uses doesn't hook it up. Don't trust its `True`, and don't claim a button works until the
binding is verified.
...
clean; just relaunch, wait, MCP auto-reconnects). This happens at **edit-time** (not just PIE) — same root
cause family as §1.2's world-lifecycle crashes: these engine ops assume the normal main-loop tick, not a
nested task context.
...
- **VibeUE** (native UE MCP, default `http://127.0.0.1:8000/mcp`) — **primary**. `manage_skills` (load
  the skills above), `execute_python_code` (arbitrary `unreal.*` + `BlueprintService`/`WidgetService`/
  `ActorService`/`ScreenshotService`), `manage_asset` (search/find/open/save/duplicate/move/delete).
  Best for Blueprint graphs, materials, UMG, assets.
- **UnrealClaude** (HTTP `:3000`, its `mcp-bridge`) — inline viewport **screenshots**
  (`unreal_capture_viewport`) and gimbal-lock-free actor **rotation/move** (`unreal_move_actor`). Use
  for the two things in §1.3. Also the source of this project's in-editor Claude Code chat panel.
- **ue-python-cli** via Bash, *if installed* — `ue-python.exe exec "<python>"` for PIE control, asset
  creation, any `unreal.*` when MCP isn't exposing what you need. Not confirmed present in this repo;
  fall back to `execute_python_code` through VibeUE if it isn't.

If VibeUE is NOT connected (server not started, or `.mcp.json` not generated yet — see the note at the
top of this file), its skills are unreachable — say so and fall back to UnrealClaude + this file.

---

## 3. Verification loop (mixed C++/Blueprint project)

1. Edit BP (VibeUE `BlueprintService`) or assets (Python) — **load the matching VibeUE skill first**.
   For C++ gameplay changes, build via Visual Studio / Live Coding per
   [Build & iterate](../../CLAUDE.md#build--iterate) instead.
...
7. Read `Saved/Logs/sword_ue5_8.log` for PrintString/UE_LOG output.
